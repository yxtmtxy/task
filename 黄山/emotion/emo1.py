...
def weighted_value(request):
    result_dict = []
    if request == "one":
        result_dict = read_file(r"D:\python\程序\黄山\emotion\主张词语（中文）.txt")
    elif request == "two":
        result_dict = read_file(r"D:\python\程序\黄山\emotion\正面评价词语（中文）.txt")
    elif request == "three":
        result_dict = read_file(r"D:\python\程序\黄山\emotion\程度级别词语（中文）.txt")
    elif request == "four":
        result_dict = read_file(r"D:\python\程序\黄山\emotion\负面情感词语（中文）.txt")
    elif request == "five":
        result_dict = read_file(r"D:\python\程序\黄山\emotion\负面评价词语（中文）.txt")
    elif request == "six":
        result_dict = read_file(r"D:\python\程序\黄山\emotion\主张词语（中文）.txt")
    else:
        pass
...
usewords = openpyxl.load_workbook(r"D:\python\程序\黄山\analyse\sum1.xlsx")
emotion = openpyxl.load_workbook(r"D:\python\程序\黄山\emotion\sum_em1.xlsx")
...
def depart(sheet, l_max):
    ...
    while row1 <= l_max:
        ...
        while cell.value is not None:
            ...
            i += 1
            cell = sheet.cell(row=row1, column=col)
        sheet0.cell(row1 + 1, 3).value = str(po_value)  # 数据存贮进单元格
        sheet0.cell(row1 + 1, 4).value = str(ne_value)
        value[row1] = po_value + ne_value
        sheet0.cell(row1 + 1, 2).value = str(value[row1])
